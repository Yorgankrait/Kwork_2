...
class InDataView(APIView):
    ...
    def post(self, request, *args, **kwargs):        
        ...
        try:
            ...
            if 'json_in' in jsonData:
                InData.objects.create(json_in=jsonData['json_in'])
                return JsonResponse({'result': 'offer insert'}, safe=False)
            else:
                return JsonResponse({'error': 'json_in not found'}, status=400)
...
class WhView(APIView):
    def post(self, request, *args, **kwargs):        
        ...
        try:
            ...
            jsonData = json.loads(postData)
            if 'url' in jsonData:
                Wh.objects.create(url=jsonData['url'])
                return JsonResponse({'result': 'url accepted'}, safe=False)
            else:
                return JsonResponse({'error': 'json_in not found'}, status=400)
...
def order_detail(request, order_number):
    ...
    # Получаем связанные продукты, опции и услуги
    products = order.product_set.all()  # Получаем все продукты, связанные с заказом
    services = order.service_set.all()   # Получаем все услуги, связанные с заказом
    upsales = order.upsale_set.all()   # Получаем все услуги, связанные с заказом
